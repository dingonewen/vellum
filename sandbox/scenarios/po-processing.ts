...
export const scenario: Scenario = {
  ...
  initialContext: {
    ...
    buyer_email: '',
    seller_email: '',
    po_number: `PO-2026-${String(Math.floor(Math.random() * 9000) + 1000)}`,
    ...
    tracking_number: `1Z${Math.random().toString(36).slice(2, 18).toUpperCase()}`,
    discount_offer: '5%',
  },
  steps: [
    // ── Step 0: Tifa sends PO with PDF attachment ─────────────────
    {
      ...
      bodyTemplate: `<p>Dear ${seller_name},</p>
...
<p>Regards,<br/><strong>${buyer_name}</strong><br/>Procurement Manager<br/>Shinra Manufacturing</p>`,
    ...
    // ── Step 1: Cloud confirms ──────────────────────────────────
    {
      ...
      bodyTemplate: `<p>Dear ${buyer_name},</p>
...
<p>${buyer_name}</p>`,
      delaySeconds: 0, // no delay — this represents days later
    },
